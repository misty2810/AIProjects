...
# Maze: . empty, # wall, K key, A1/A2 agents
maze = [
    list("A1..K."),
    list(".#..K."),
    list(".K..A2"),
...
def bfs_collect(start, cols_allowed):
    ...
    while q:
        ...
        steps += 1
        if (r,c) in keys:
            keys.remove((r,c)); collected += 1
        for dr,dc in [(1,0),(-1,0),(0,1),(0,-1)]:
            nr,nc = r+dr,c+dc
            if 0<=nr<ROWS and 0<=nc<COLS and maze[nr][nc] != "#" and not visited[nr][nc] and nc in cols_allowed:
                visited[nr][nc] = True
                q.append((nr,nc))
...
# visualization: mark visited V (but keep agents/keys)
out = []
for r in range(ROWS):
    row = ""
    for c in range(COLS):
